...
public class EricMiniMaxDecision extends Player implements agent.Agent, Serializable {
	...
	public EricMiniMaxDecision(boolean isRed) {
		super(isRed, "Eric - MiniMax Agent + Decision");
		filename = "testfile";
		...
		DataDTreeGenerator gen = new DataDTreeGenerator("gamedata.0", "");
		try {
	...
	public Action getAction(Percept cP) {
		 ...
		 if(board.getMovesSoFar().size() < 2) {
			 this.black = board.getBlackPieces();
			 this.white = board.getRedPieces();
		 }
		 ...
		 MiniMaxDecision search = new MiniMaxDecision(this.nodeInfo, this.tree, this.black, this.white);
